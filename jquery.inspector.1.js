(function (owner) {
    ...
    function ComponentWatcher($el, instanceName, className, fnclass) {
        ...
        $el._replaceThisWithMyInstanceName = function (_html) {


            // REMPLAZA LOS THIS POR MI INSTANCENAME     
            _html = _html || $el.html();
           ...
           return new Promise((resolve,reject) => {
                $el.html(_html.split("this.").join(instanceName+"."));

                resolve();
                /*
                _iterateDom($el.get(0));
                var timeoutToCallEndIteration = null;
                function _iterateDom(_el) {
                    if (_el.nodeName == "#text") {
                        // SI NO TIENE ATRIBUTOS Y ES UN TEXTO VALIDA EL TEXTCONTEN DE ESTE NODO.
                        _el.textContent = _el.textContent.split("this.").join(instanceName)
                    } else {
                        var _continue = true;
                        if ($el.get(0) != _el) {
                            for (let i in Inspector.attrselector) {
                                let s = Inspector.attrselector[i];
                                if (_el.attributes.hasOwnProperty(s)) {
                                    // IF HAS an component then validate if 
                                    _continue = false;
                                    break;
                                }
                            }
                        ...
                        if (_continue) {
                            for (let i in _el.attributes) {
                                
                                if (!isNaN(i) && _el.attributes[i].value != null && _el.attributes[i].value.indexOf("this.") > -1) {
                                    console.log("attr check:",i,_el.attributes[i])
                                    _el.attributes[i].value = _el.attributes[i].value.split("this.").join(instanceName + ".")
                                }
                                //console.log(_el.attributes)
                            }
                            // VALIDATE CHILDNODES
                    ...
                    clearTimeout(timeoutToCallEndIteration);
                    timeoutToCallEndIteration = setTimeout(_endIteration, 100);
                }

                function _endIteration() {
                      $el.get(0).Inspector.inspect();
                    setTimeout(() => {
        ...
        $el.setHtml = $el.setHTML = (html) => {
            $el._replaceThisWithMyInstanceName(html);
        };
        ...
        $el._ready = false;
        $el._replaceThisWithMyInstanceName().then(() => {
        ...
        // AGREA PROPIEDADES AL EVALUADOR
